...
from models_search.building_blocks_search import CONV_TYPE, NORM_TYPE, UP_TYPE, SHORT_CUT_TYPE, SKIP_TYPE, DISCONV_TYPE
...
class Controller(nn.Module):
    def __init__(self, args, cur_stage):
        ...
        if cur_stage:
            self.tokens = [len(CONV_TYPE), len(NORM_TYPE), len(UP_TYPE), len(SHORT_CUT_TYPE), len(SKIP_TYPE)**cur_stage,
                           len(DISCONV_TYPE), len(NORM_TYPE), len(SKIP_TYPE)**cur_stage]
        else:
            self.tokens = [len(CONV_TYPE), len(NORM_TYPE), len(UP_TYPE), len(SHORT_CUT_TYPE), len(DISCONV_TYPE),
                           len(NORM_TYPE)]
    ...
    def forward(self, x, hidden, index):
        ...
        return logit, (hx, cx)  # logit是一个参数向量,长度为相应token的值

    def sample(self, batch_size, with_hidden=False, prev_hiddens=None, prev_archs=None):
        x = self.initHidden(batch_size)  # (batch_size, hid_size)
        ...
        if prev_hiddens:
            ...
            selected_archs = torch.cat(selected_archs, 0)   # 从前面的controller里选取topk中的一组参数
            hidden = (torch.cat(selected_hxs, 0), torch.cat(selected_cxs, 0))
        else:
            hidden = (self.initHidden(batch_size), self.initHidden(batch_size))  # stage=0即初始时是没有pre_h和pre_c的,只能初始化
        entropies = []
        actions = []
        selected_log_probs = []
        for decode_idx in range(len(self.decoders)):  # controller参数的个数
        ...
        if prev_hiddens:
            archs = torch.cat([selected_archs, archs], -1)  # 把之前的controller的参数和当前的controller参数连接起来

        if with_hidden:  # 应该是最后一个不用传hidden
