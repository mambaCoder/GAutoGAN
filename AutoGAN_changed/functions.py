...
def train_shared(args, gen_net: nn.Module, dis_net: nn.Module, g_loss_history, d_loss_history, controller, gen_optimizer
    ...
    # eval mode
    controller.eval()
    for epoch in range(args.shared_epoch):  # 15
        for iter_idx, (imgs, _) in enumerate(train_loader):

            # sample an arch
            arch = controller.sample(1, prev_hiddens=prev_hiddens, prev_archs=prev_archs)[0][0]  # 为啥是[0][0]???
            gen_net.set_arch(arch, controller.cur_stage)
            dis_net.set_arch(arch, controller.cur_stage)
...
# train_controller和get_topk_arch_hidden一样可以在参数中加入dis_net并用WGAN作为reward, dis_net.eval()
...
def validate(args, fixed_z, fid_stat, gen_net: nn.Module, writer_dict, clean_dir=True):  # train_derived会用到
    writer = writer_dict['writer']
...
# def get_topk_arch_hidden(args, controller, gen_net, dis_net, prev_archs, prev_hiddens):  # 可以改成w距离作为reward
#     """
#     ~
#     :param args:
#     :param controller:
