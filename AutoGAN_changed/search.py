...
def main():
    ...
    # set grow controller
    grow_ctrler = GrowCtrler(args.grow_step1, args.grow_step2)  # 15 35 用来判断当前处于哪个stage

    # initial
    start_search_iter = 0

    # set writer 可能从之前保存的检查点开始读入模型继续上次的训练
    if args.load_path:
    ...
    g_loss_history = RunningStats(args.dynamic_reset_window)  # 500
    ...
    for search_iter in tqdm(range(int(start_search_iter), int(args.max_search_iter)), desc='search progress'):
        ...
        if search_iter == args.grow_step1 or search_iter == args.grow_step2:
            ...
            dataset = datasets.ImageDataset(args, 2 ** (cur_stage + 3))  # 提高分辨率训练，此时训练集也应该改变
            train_loader = dataset.train
